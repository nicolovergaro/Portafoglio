      ...
      <div class="row select_section">
        <div class="col col-50 padding_right_5px">
          <label class="item item-input item-select month_select">
            ...
            <select ng-model="data.currentMonth" ng-options="item.text for item in monthsList.months" ng-change="monthChanged(data.currentMonth.value)">
            </select>
          </label>
        </div>
    ...
    <div>
      <div class="row text-center padding_zero" ng-repeat="row in rows track by $index">
        <div class="col no_padding date_col" ng-repeat="col in cols track by $index"
             ng-class="{'selected_date': ((mainObj.selectMode === 'day' && dayList[row + $index].epoch === selctedDateEpoch)
                 || (mainObj.selectMode !== 'day' && dayList[row + $index].epoch >= selctedDateEpoch && dayList[row + $index].epoch <= selctedDateEpochEndWeek)),
